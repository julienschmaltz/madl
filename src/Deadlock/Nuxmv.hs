...
generateInvariants net invs = mapMaybe invToNuxmv invs where
    ...
    invToNuxmv inv@(Invariant qMap aMap c) = if emptyInvariant inv
        then Nothing
        else Just $ nuxmv_invar $ nuxmv_equals "0" (nuxmv_add invariantClauses) where
            invariantClauses = concatMap (uncurry qToNuxmv) (Map.assocs qMap)
...
callEngine engine = do
    ...
    when (isAbcEngine engine) $ executeProcess "nuxmv -source" writeAiger ""
