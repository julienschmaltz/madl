...
export_row_to_smt x show_p_t inv@(Invariant qMap aMap c) = if emptyInvariant inv then "" else smt_assert $ smt_equals "0" invar where
...
-- | Declare an queue nr of packets invariant
...
smt_queue, smt_loadbalancer_arbiter, smt_merge_arbiter, smt_match_arbiter_m, smt_match_arbiter_d :: ColoredNetwork -> ComponentID -> String
smt_automaton_state, smt_automaton_arbiter_c, smt_automaton_arbiter_t :: ColoredNetwork -> ComponentID -> String
-- For queue variables, we add "Q__" in front to make it easy at the UI level to find queue variables
smt_queue x i = "Q___" ++ (utxt . getName $ getComponent x i) -- ++ "___" ++ (utxt $ channelName $ fst $ getChannel x ((getOutChannels x i)!!0))
smt_loadbalancer_arbiter x i = utxt . getName $ getComponent x i
-- For merge variables, we add "M__" in front to make it easy at the UI level to find merge variables
...
smt_automaton_state x i = (utxt . getName $ getComponent x i) ++ "_state"
smt_automaton_arbiter_c x i = (utxt . getName $ getComponent x i) ++ "_c"
smt_automaton_arbiter_t x i = (utxt . getName $ getComponent x i) ++ "_t"
...
showModel = unlines . mapMaybe (uncurry showVariable) . Map.assocs where
    showVariable var val
        ...
        | isPrefixOf ("idle_") var = Nothing -- if (val == 1) then Just $ var ++ " := true" else Nothing --don't show idle variables
