...
emptyInvariant (Invariant qMap aMap c) = Map.null qMap && Map.null aMap && c == 0
...
cleanInvariant net (Invariant qMap aMap c) = (Invariant (Map.mapWithKey annotateType qMap) aMap c) where
...
toIntInvariant (Invariant qMap aMap c) = Invariant (fmap (fmap toInt) qMap) (fmap (fmap toInt) aMap) (toInt c) where
    toInt :: Ratio Int -> Int
    toInt (n :% d) = n * (f `div` d)
...
showInvariant nameFunction (Invariant qMap aMap c) = "0 = "
    +++ T.intercalate " + " (map showQ (Map.assocs qMap) ++ map showA (Map.assocs aMap) ++ [showT c | c /= 0]) where
