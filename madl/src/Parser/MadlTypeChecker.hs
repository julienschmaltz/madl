...
typecheckNetworkPrimitive context networkprimitive = case removeSourceInfo networkprimitive of
    ControlJoin a b -> combine (Just 1) (fmap fst checkedChannels) checkedPrimitive2 where
        checkedPrimitive2 = join $ liftM2 (checkInputOutputSizes [Just 1, Just 1]) (sizes checkedChannels) checkedPrimitive
        checkedPrimitive = fmap setSource $ liftM2 ControlJoin (chan 0 checkedChannels) (chan 1 checkedChannels)
        checkedChannels = typecheckChannelExpressions context $ setSource [a, b]
    DeadSink i -> combine (Just 0) (fmap fst checkedChannels) checkedPrimitive2 where
        checkedPrimitive2 = join $ liftM2 (checkInputOutputSizes [Just 1]) (sizes checkedChannels) checkedPrimitive
        checkedPrimitive = fmap (setSource . DeadSink) (chan 0 checkedChannels)
        checkedChannels = typecheckChannelExpressions context $ setSource [i]
    FControlJoin p a b -> combine (Just 1) (fmap fst checkedChannels) checkedPrimitive2 where
        checkedPrimitive2 = join $ liftM2 (checkInputOutputSizes [Just 1, Just 1]) (sizes checkedChannels) checkedPrimitive
        checkedPrimitive = fmap setSource $ liftM3 FControlJoin checkedPredicate (chan 0 checkedChannels) (chan 1 checkedChannels)
        checkedPredicate = checkPredicateInputSize context 2 =<< fmap setSource (typecheckPredicateExpression context $ setSource p)
        checkedChannels = typecheckChannelExpressions context $ setSource [a, b]
    Fork i -> combine Nothing (fmap fst checkedChannels) checkedPrimitive2 where
        checkedPrimitive2 = join $ liftM2 (checkInputOutputSizes [Just 1]) (sizes checkedChannels) checkedPrimitive
        checkedPrimitive = fmap (setSource . Fork) (chan 0 checkedChannels)
        checkedChannels = typecheckChannelExpressions context $ setSource [i]
    Function f i -> combine (Just 1) (fmap fst checkedChannels) checkedPrimitive2 where
        checkedPrimitive2 = join $ liftM2 (checkInputOutputSizes [Just 1]) (sizes checkedChannels) checkedPrimitive
        checkedPrimitive = fmap setSource $ liftM2 Function checkedFunction (chan 0 checkedChannels)
        checkedFunction = checkFunctionInputSize context 1 =<< fmap setSource (typecheckFunctionExpression context $ setSource f)
        checkedChannels = typecheckChannelExpressions context $ setSource [i]
    Joitch a b jss -> combine (Just $ 2 * length jss) (fmap fst checkedChannels) checkedPrimitive2 where
        checkedPrimitive2 = join $ liftM2 (checkInputOutputSizes [Just 1, Just 1]) (sizes checkedChannels) checkedPrimitive
        checkedPrimitive = fmap setSource $ liftM3 Joitch (chan 0 checkedChannels) (chan 1 checkedChannels) checkedJoitchExpressions
        checkedJoitchExpressions = mapM (typecheckJoitchExpression context . setSource) =<< typecheckJoitchOtherwise (setSource jss)
        checkedChannels = typecheckChannelExpressions context $ setSource [a, b]
    LoadBalancer i -> combine Nothing (fmap fst checkedChannels) checkedPrimitive2 where
        checkedPrimitive2 = join $ liftM2 (checkInputOutputSizes [Just 1]) (sizes checkedChannels) checkedPrimitive
        checkedPrimitive = fmap (setSource . LoadBalancer) (chan 0 checkedChannels)
        checkedChannels = typecheckChannelExpressions context $ setSource [i]
    Match p a b -> combine (Just 2) (fmap fst checkedChannels) checkedPrimitive2 where
        checkedPrimitive2 = join $ liftM2 (checkInputOutputSizes [Just 1, Just 1]) (sizes checkedChannels) checkedPrimitive
        checkedPrimitive = fmap setSource $ liftM3 Match checkedPredicate (chan 0 checkedChannels) (chan 1 checkedChannels)
        checkedPredicate = checkPredicateInputSize context 2 =<< fmap setSource (typecheckPredicateExpression context $ setSource p)
        checkedChannels = typecheckChannelExpressions context $ setSource [a, b]
    Merge a b l -> combine (Just 1) (fmap fst checkedChannels) checkedPrimitive2 where
        ...
        checkedChannels = typecheckChannelExpressions context $ setSource [a, b]
    MultiMatch p a -> combine Nothing (fmap fst checkedChannels) checkedPrimitive2 where --todo(tssb) check total input size at least 2
        checkedPrimitive2 = join $ liftM2 (checkInputOutputSizes $ repeat Nothing) (sizes checkedChannels) checkedPrimitive
        checkedPrimitive = fmap setSource $ liftM2 MultiMatch checkedPredicate (chans checkedChannels)
        checkedPredicate = checkPredicateInputSize context 2 =<< fmap setSource (typecheckPredicateExpression context $ setSource p)
        checkedChannels = typecheckChannelExpressions context $ setSource a
    PatientSource e -> combine (Just 1) (Right context) (fmap PatientSource . typecheckTypeExpression context $ setSource e)
    Queue k i l -> combine (Just 1) (fmap fst checkedChannels) checkedPrimitive2 where
        ...
        checkedChannels = typecheckChannelExpressions context $ setSource [i]
    Sink i -> combine (Just 0) (fmap fst checkedChannels) checkedPrimitive2 where
        checkedPrimitive2 = join $ liftM2 (checkInputOutputSizes [Just 1]) (sizes checkedChannels) checkedPrimitive
        checkedPrimitive = fmap (setSource . Sink) (chan 0 checkedChannels)
        checkedChannels = typecheckChannelExpressions context $ setSource [i]
    Source e -> combine (Just 1) (Right context) (fmap Source . typecheckTypeExpression context $ setSource e)
    Switch i sws -> combine (Just $ length sws) (fmap fst checkedChannels) checkedPrimitive2 where
        checkedPrimitive2 = join $ liftM2 (checkInputOutputSizes [Just 1]) (sizes checkedChannels) checkedPrimitive
        checkedPrimitive = fmap setSource $ liftM2 Switch (chan 0 checkedChannels) checkedSwitchExpressions
        checkedSwitchExpressions = mapM (typecheckSwitchExpression context . setSource) =<< typecheckSwitchOtherwise (setSource sws)
        checkedChannels = typecheckChannelExpressions context $ setSource [i]
    Vars i -> combine (Just 1) (fmap fst checkedChannels) checkedPrimitive2 where
        checkedPrimitive2 = join $ liftM2 (checkInputOutputSizes [Just 1]) (sizes checkedChannels) checkedPrimitive
        checkedPrimitive = fmap (setSource . Vars) (chan 0 checkedChannels)
        checkedChannels = typecheckChannelExpressions context $ setSource [i]
