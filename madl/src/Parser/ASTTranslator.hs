...
getPrimitiveArguments (ControlJoin a b)             = [PrimitiveChannel a, PrimitiveChannel b]
getPrimitiveArguments (DeadSink i)                  = [PrimitiveChannel i]
getPrimitiveArguments (FControlJoin f a b)          = [PrimitivePredicate f, PrimitiveChannel a, PrimitiveChannel b]
getPrimitiveArguments (Fork i)                      = [PrimitiveChannel i]
getPrimitiveArguments (Function f i)                = [PrimitiveFunction f, PrimitiveChannel i]
getPrimitiveArguments (Joitch a b ps)               = [PrimitiveChannel a, PrimitiveChannel b] ++ map PrimitiveJoitch ps
getPrimitiveArguments (LoadBalancer i)              = [PrimitiveChannel i]
getPrimitiveArguments (Match f mIn dIn)             = [PrimitivePredicate f, PrimitiveChannel mIn, PrimitiveChannel dIn]
getPrimitiveArguments (Merge a b _)                 = [PrimitiveChannel a, PrimitiveChannel b]
getPrimitiveArguments (MultiMatch f ins)            = (PrimitivePredicate f : map PrimitiveChannel ins)
getPrimitiveArguments (PatientSource e)             = [PrimitiveType e]
getPrimitiveArguments (Queue k i _)                 = [PrimitiveInteger k, PrimitiveChannel i]
getPrimitiveArguments (Sink i)                      = [PrimitiveChannel i]
getPrimitiveArguments (Source e)                    = [PrimitiveType e]
getPrimitiveArguments (Switch i sws)                = (PrimitiveChannel i : map PrimitiveSwitch sws)
getPrimitiveArguments (Vars i)                      = [PrimitiveChannel i]
...
addPrimitive spec networkprimitiveSrc name context = addComponents [(component, srcinfo)] spec' where
    ...
    (component, spec') = case removeSourceInfo networkprimitiveSrc of
        ControlJoin                    _ _        -> (M.C $ M.ControlJoin name, spec)
        DeadSink                       _          -> (M.C $ M.DeadSink name, spec)
        FControlJoin                   f _ _      -> (M.C $ M.FControlJoin name function, spec) where
                                                        function = evaluatePredicateExpression context f
        Fork                           _          -> (M.C $ M.Fork name, spec)
        Function                       f _        -> (M.C $ M.Function name function returntype, spec) where
                                                        (function, returntype) = evaluateFunctionExpression context f
        Joitch                         _ _ ps     -> (M.C $ M.Joitch name preds, spec) where
                                                        preds = map (evaluateJoitchExpression context ps) ps
        LoadBalancer                   _          -> (M.C $ M.LoadBalancer name, spec)
        Match                          f _ _      -> (M.C $ M.Match name function, spec) where
                                                        function = evaluatePredicateExpression context f
        Merge                          _ _ _      -> (M.C $ M.Merge name, spec)
        MultiMatch                     f _        -> (M.C $ M.MultiMatch name function, spec) where
                                                        function = evaluatePredicateExpression context f
        PatientSource                  e          -> (M.C $ M.PatientSource name msgtype, spec) where
        ...
        Sink                           _          -> (M.C $ M.Sink name, spec)
        Source                         e          -> (M.C $ M.Source name msgtype, spec) where
                                                        msgtype = evaluateTypeExpression context e
        Switch                         _ ps       -> (M.C $ M.Switch name swtypes, spec) where
                                                        swtypes = map (evaluateSwitchExpression context ps) ps
        Vars                           _          -> (M.C $ M.Vars name, spec)
